...
## MP-24: Preserve runner evidence when process cleanup is denied

A process-group cleanup permission error must not escape before the attempted turn and timeout/exit evidence are recorded. Preserve the denial as a failure, stop that cleanup operation without PID fallbacks, privilege changes or repeated denied signals, and report that child termination is unconfirmed. Keep exact-model, valid-session, retry-budget and apply gates fail-closed. Existing interrupted tasks remain unchanged. Scope is bounded runner error handling, focused synthetic tests, operational guidance and publication; no sandbox weakening or automatic recovery. SWE-2 High owns `swe_sidekick.py` and `tests/test_sidekick.py`; lead owns plans, review, documentation, installation and GitHub merge. Completion requires synthetic timeout/normal-exit/permission-denial coverage, existing safety regressions, retained turn evidence, and a reviewed published/installed patch.
